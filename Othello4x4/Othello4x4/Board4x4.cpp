...
bitboard_t getRev(bitboard_t black, bitboard_t white, bitboard_t p)		//	黒を p に打った場合に、反転する白のパターンを取得
{
	...
	//	下方向に返る石をチェック
	rev |= (p >> 4) & wv & (black << 4);    //  ・○● の場合
	rev |= (p >> 4) & wv & (wv << 4) & (black << 8);    //  ・○○● の場合の空欄隣の○
	rev |= (p >> 8) & (wv >> 4) & wv & (black << 4);    //  ・○○● の場合の次の○
	//  上方向に返る石をチェック
	rev |= (p << 4) & wv & (black >> 4);    //  ●○・ の場合
	rev |= (p << 4) & wv & (wv >> 4) & (black >> 8);    //  ●○○・ の場合の空欄隣の○
	rev |= (p << 8) & (wv << 4) & wv & (black >> 4);    //  ●○○・ の場合の次の○
	...
	//	左下方向に返る石をチェック
	rev |= (p >> 3) & wd & (black << 3);    //  ・○● の場合
	rev |= (p >> 3) & wd & (wd << 3) & (black << 6);    //  ・○○● の場合の空欄隣の○
	rev |= (p >> 6) & (wd >> 3) & wd & (black << 3);    //  ・○○● の場合の次の○
	//	右下方向に返る石をチェック
	rev |= (p >> 5) & wd & (black << 5);    //  ・○● の場合
	rev |= (p >> 5) & wd & (wd << 5) & (black << 10);    //  ・○○● の場合の空欄隣の○
	rev |= (p >> 10) & (wd >> 5) & wd & (black << 5);    //  ・○○● の場合の次の○
	//	左上方向に返る石をチェック
	rev |= (p << 5) & wd & (black >> 5);    //  ●○・ の場合
	rev |= (p << 5) & wd & (wd >> 5) & (black >> 10);    //  ●○○・ の場合の空欄隣の○
	rev |= (p << 10) & (wd << 5) & wd & (black >> 5);    //  ●○○・ の場合の次の○
